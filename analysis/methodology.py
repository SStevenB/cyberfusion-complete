...
DATA_SOURCES = {
    ...
    "UploadedNmap": {
        "name":        "Uploaded Nmap Scan (XML)",
        "url":         "https://nmap.org/book/output-formats-xml-output.html",
        "type":        "User-uploaded file (authorized scan)",
        "license":     "User-provided; authorized targets only",
        "description": "nmap -oX XML output uploaded by the user. Parsed into open-port scan findings. CyberFusion does not run this scan — the user provides it for systems they are authorized to assess.",
        "refresh":     "On upload",
    },
    "UploadedVulnScan": {
        "name":        "Uploaded Vulnerability Scan (CSV)",
        "url":         "",
        "type":        "User-uploaded file (Nessus/Tenable/Qualys/OpenVAS export)",
        "license":     "User-provided",
        "description": "Vulnerability-scanner CSV export uploaded by the user. Parsed into vulnerability findings (CVE, CVSS, affected host) that feed the web-CVE and KEV correlation rules.",
        "refresh":     "On upload",
    },
    "UploadedAssetInventory": {
        "name":        "Uploaded Asset Inventory (CSV)",
        "url":         "",
        "type":        "User-uploaded file (CMDB / spreadsheet export)",
        "license":     "User-provided",
        "description": "Asset inventory CSV uploaded by the user. Supplies asset criticality tiers (1–3) that drive the score multiplier so findings on crown-jewel assets are prioritized.",
        "refresh":     "On upload",
    },
    "UploadedBreachExport": {
        "name":        "Uploaded Breach Export (CSV)",
        "url":         "https://haveibeenpwned.com/",
        "type":        "User-uploaded file (HaveIBeenPwned domain search)",
        "license":     "User-provided; domains the user controls",
        "description": "HaveIBeenPwned-style breach export uploaded by the user for a domain they own. Parsed into breach signals feeding the credential-stuffing and corporate-email-exposure rules.",
        "refresh":     "On upload",
    },
    "Synthetic": {
